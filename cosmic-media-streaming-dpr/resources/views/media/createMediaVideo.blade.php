...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            let initialChunkSize = 15 * 1024 * 1024; // 15MB starting point
            let currentChunkSize = initialChunkSize;
            let minChunkSize = 5 * 1024 * 1024; // 5MB minimum
            let maxChunkSize = 30 * 1024 * 1024; // 30MB maximum
            ...
            function calculateSpeed(uploadBytes) {
                ...
                // Update adaptive chunk size based on current speed
            ...
            submitButton.on('click', async function() {
                ...
                try {
                    Swal.fire({
                        title: 'Checking...',
                        text: 'Validating video name and file',
                    ...
                    const checkResponse = await fetch((
                        `{!! env('URL_APP') !!}/api/checkVideoName?name=${encodeURIComponent(videoName)}&fileName=${encodeURIComponent(fileName)}`
                    ));
                    ...
                    if (checkResult.nameExists) {
                        Swal.fire({
                            ...
                            text: 'A video with this name already exists. Please choose a different name.',
                            icon: 'warning',
                    ...
                    if (checkResult.fileExists) {
                        Swal.fire({
                            ...
                            text: 'A file with this name already exists in storage. Please rename your file before uploading.',
                            icon: 'warning',
                            confirmButtonText: 'OK'
                        });
                        return;
                    ...
                    Swal.fire('Error', 'Failed to validate video name.', 'error');
                }
            });
            ...
            resumable.on('fileSuccess', async function(file, response) {
                ...
                try {
                    const apiResponse = await fetch("{!! env('URL_APP') !!}/api/storeCreateMediaVideo",
                        requestOptions);
                    ...
                    Swal.fire({
                        position: "center",
                        icon: "success",
                        title: "Create video successfully!",
            ...
            resumable.on('chunkError', function(file, message) {
                ...
                if (isOnline && autoRetryCount < maxAutoRetries) {
                    autoRetryCount++;
                    console.log(`Auto-retrying chunk upload (${autoRetryCount}/${maxAutoRetries})...`);
                    setTimeout(() => {
                        resumable.upload();
                    }, 2000);
                } else if (autoRetryCount >= maxAutoRetries) {
                    console.error(`Maximum retry attempts (${maxAutoRetries}) reached.`);
                    Swal.fire('Error',
                        `Upload failed after ${maxAutoRetries} retry attempts. Please try again later.`,
                        'error');
                    hideProgress();
                    uploadSpeed.hide();
                }
            ...
            function hideProgress() {
                ...
                uploadSpeed.hide();
                uploadControls.hide();
                pauseButton.prop('hidden', true);
                resumeButton.prop('hidden', true);
            }

            function cancel() {
                window.location.href = "{!! env('URL_APP') . '/back-office/media-videos' !!}";
            }
        });
    </script>
</body>
